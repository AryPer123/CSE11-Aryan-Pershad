# CSE 11 Spring 2026 PA4 - Airline Reservation System
**Due date: Thursday, May 7 @ 11:59PM PDT**
...
Airline reservation systems are systems that allow an airline to sell their seats on a flight. In this programming assignment, we will implement a simple airline reservation system that allows the user to book, cancel, and upgrade tickets. They can also look up a passenger's seat (row) number, print the number of available tickets in each travel class, and print a view of the plane with passenger names.
...
The following lines populate the plane with existing passengers. Here is a visualization of how the plane is initialized:
```
...
We will pass the CSV file name through command line arguments and utilize Java's `Scanner` class to read from the CSV file. You can also assume that passenger names will not contain commas, be an empty string, or be pure whitespaces.
...
> ***Do NOT add any additional static variables. (Unless they are private static final variables of primitive or String types, which are treated as constants.)***
...
- `public static boolean upgrade(String passengerName, int upgradeClass)`
    ...
    - If `upgradeClass` is of lower or equal status to `passengerName`'s existing class (which is larger than or equal to in integer value), return `false` and do not modify `passengers`. *(i.e. a First Class passenger trying to 'upgrade' to Business Class)*
...
- `public static int[] greedyAssign(int budget, String[] toBook)`
    - Given a `budget` and an array `toBook` of passenger names to assign seats, greedily book all passengers into the best possible travel class within the budget.
    - Each seat has a fixed price per class: Economy Class costs `ECONOMY_CLASS_PRICE` (100), Business Class costs `BUSINESS_CLASS_PRICE` (200), and First Class costs `FIRST_CLASS_PRICE` (400). ***These constants are already defined for you in the starter code.***
    - The **greedy strategy** is to maximize the number of passengers placed in the highest-tier class first, then the next-highest, and so on — subject to both the budget and available seats in each class. Passengers not upgraded stay in Economy Class.
    - *Hint: think of each passenger as having a baseline Economy seat. Upgrading a passenger to a higher class only costs the **difference** in price — for example, a First Class upgrade costs `FIRST_CLASS_PRICE - ECONOMY_CLASS_PRICE` extra per person.*
    - Once the class assignment counts are determined, book all passengers in the order they appear in `toBook` (First Class passengers first, then Business, then Economy), using methods you have already implemented.
    - Return an `int[]` of length 3 in the format `[firstClassCount, businessClassCount, economyClassCount]`.
    - You may assume `budget >= toBook.length × ECONOMY_CLASS_PRICE` (everyone is guaranteed to fit in Economy Class by both capacity and budget).
    - You may assume `toBook` is non-null, contains no null entries, and all names are not already booked.
...
Submit the following file(s) to Gradescope by **Thursday, May 7 @ 11:59PM PDT**.
...
- You are only allowed to call methods in the class `AirlineReservation` and in library classes listed below.
  Calling methods in other library classes will result in a 0.
    ...
    - `java.lang.String`
    - `java.util.Scanner`
- You are only allowed to access fields (i.e. class variables and instance variables) in the class `AirlineReservation` and in library classes listed below.
  Accessing fields in other library classes will result in a 0.
    - `java.lang.System`
